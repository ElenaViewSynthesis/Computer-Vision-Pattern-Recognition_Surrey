...
precisionRecallCurve_constructor(dst); 
%queryClass;
groundTruthVector = zeros(NIMG, 1); % Assuming all images are initially irrelevant

% Top 5 are relevant images in the ground truth
relevantImages = dst(1:5, 2); % Need to adjust based on the ACTUAL RELEVANCE criteria
groundTruthVector(relevantImages)=1;

% Initialise arrays to store precision and recall values
precision = zeros(NIMG, 1);
recall = zeros(NIMG, 1);

% Compute precision and recall at each threshold
for i = 1:NIMG
    retrievedImages = dst(1:i, 2);
